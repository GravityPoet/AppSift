...
<p align="center">
  <b>夺回被焊死的 Mac 空间。离线优先、零遥测、100% 开源的 Mac 清理与卸载神器，终结高昂订阅。</b>
</p>
...
- **社区版（AGPL-3.0）**：任何个人或企业均可依照 AGPL-3.0 免费使用、修改和分发。拥有完整功能的应用卸载、残留清理、开发者与 AI 缓存清理，且零遥测跟踪。
- **商业双重授权**：需要闭源集成、免除 AGPL 开源义务、专有再分发、OEM 授权或其他专有使用权的组织，可联系 GravityPoet 获取商业许可证。

---

## 🎯 为什么我们需要这个项目？

因为苹果高昂的固态硬盘升级费让很多人只能选择 **256GB 的低配 Mac** (如 Mac mini, MacBook Air / Pro)，而市面上的商业清理软件大多采用**昂贵的订阅制**，部分闭源清理软件包含遥测或难以独立审计的数据收集机制，还喜欢用*“发现 47 GB 危险垃圾”*这种虚假警报来恐吓你续费。

AppSift 就是为此而生的终结者。它 **100% 免费**、基于 **AGPL-3.0 协议开源**、完全**离线优先且绝无联网收集隐私**。它不搞虚头巴脑的噱头，只用最干净、最诚实的方式，帮你把被开发缓存和残留垃圾蚕食的硬盘空间一寸一寸夺回来。
...
|---|---|
| 每年花 200+ 元订阅清理软件，只为了点一下“一键清理”。 | **100% 免费开源 (AGPL-3.0)**。把预算留给真正重要的地方。 |
...
- **⚡ 智能卸载与残留清理**
  仅将应用拖入废纸篓，通常仍会留下偏好设置、缓存、容器、日志和后台组件。AppSift 深入系统底层，匹配 bundle ID 和 containers，自动识别并清理所有关联 plist、启动项和日志，而且删除默认移入垃圾桶，支持一键安全还原。
...
- **🧭 存储分析工具箱**
  用 Space Lens 深入磁盘目录，按文件内容确认完全重复项，用感知哈希和图像特征聚类普通文件夹或“照片”图库中的相似照片，并查看 iPhone/iPad 本地备份及普通下载文件的来源。扫描均可取消、有数量边界；普通文件夹跳过云占位文件并使用废纸篓恢复，“照片”图库则通过 PhotoKit 跳过仅存于 iCloud 的原片，并将确认项目移到“最近删除”。

- **🩺 隐私、维护与可解释健康建议**
  可清理浏览器历史、下载记录、Cookies 和缓存，但不碰密码、自动填充与 Wi‑Fi；按需检查 macOS 更新，修复 DNS、Spotlight 或 Mail 索引；在菜单栏监控磁盘、内存、网络、电池、外接磁盘和受支持设备。建议基于明确规则和证据，不制造虚假的“健康分数”。

- **🛡️ 安全透明，完全掌控**
  没有任何后台上传或联网分析。普通文件删除默认进入系统废纸篓，“照片”图库资产进入“最近删除”，均保留系统恢复路径；永久垃圾清理会单独明确提示。核心系统路径被硬编码锁定保护，绝不会误伤系统。
...
| **开源** | **Yes (AGPL-3.0)** | No / 闭源 | Source-available¹ | CLI only | No / 闭源 |
...
<sub>¹ Pearcleaner is Apache 2.0 **+ Commons Clause** - source-available but not OSI-approved. AppSift 采用 AGPL-3.0 开源，并提供独立的商业授权选项。</sub>
...
*   **默认可恢复，不可恢复时明确提示**：应用卸载、App Reset、安装文件和残留清理默认移入系统废纸篓并记录恢复映射。缓存、日志等可重新生成的系统清理项目，仅会在单独展示并经用户确认后永久删除。
*   **绝对零遥测**：无分析、无崩溃报告、无“匿名数据收集”。软件完全在本地离线运行。
*   **网络使用完全透明**：只有在你手动点击“检查更新”时，才会发起受限的网络请求。应用更新包括 App Store 收据、Homebrew Cask 信息以及 GitHub 官方 Releases 链接；单独的 macOS 更新检查只按需调用苹果系统的 `/usr/sbin/softwareupdate --list`，不会自行下载或安装系统更新。AppSift 绝不请求未知服务器或在后台静默下载安装包。
...
### 存储分析与设备备份
Space Lens 会为用户选择的文件夹或已挂载磁盘建立可取消的容量目录树，支持逐层进入 Treemap；扫描不跟随符号链接，并跳过云占位文件。重复文件先按大小分桶，再进行抽样哈希和完整 SHA-256 确认。相似图片结合感知哈希、Vision 特征及清晰度、分辨率、曝光、人脸质量等可解释指标，并始终保留每组中建议的最佳副本：普通文件夹使用废纸篓并可撤销；“照片”图库使用 PhotoKit 读写授权，不把图库包当作文件遍历，不下载仅存于 iCloud 的原片，把 Live Photo、RAW 与连拍资源作为完整资产处理，并只将确认项目移到“最近删除”。iPhone/iPad 备份功能读取受限的 MobileSync 本地元数据，按设备归类，可将单个已复核备份移入废纸篓并撤销。下载来源归类只展示本地 quarantine 元数据中的来源应用和域名，不显示完整 URL。

### 浏览器隐私、系统维护与健康建议
浏览器隐私首版只覆盖 Safari、Chrome 和 Firefox 的历史、下载记录、Cookies 与缓存；密码库、自动填充、书签、打开的标签、扩展和已保存 Wi‑Fi 均不在处理范围。系统维护提供明确触发、范围受限的 DNS 刷新、Spotlight 索引重建和 Mail Envelope Index 修复，其中 Mail 索引文件通过废纸篓保留恢复能力。菜单栏监控与告警中心读取本机当前磁盘、内存压力、网络速度、电池、外接磁盘、长期未清理的废纸篓以及受支持设备电量；健康建议只按公开规则呈现证据与操作，不生成缺乏依据的数字评分。
...
## 许可证 / License
基于 AGPL-3.0（开源社区版）与专有商业许可（Pro 专业版）的双重许可协议。详情请参阅 [LICENSE](../LICENSE)。
