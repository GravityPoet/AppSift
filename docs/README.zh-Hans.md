...
- **🧭 存储分析工具箱**
  用 Space Lens 深入磁盘目录，按文件内容确认完全重复项，用感知哈希和图像特征聚类相似照片，并查看 iPhone/iPad 本地备份及普通下载文件的来源。扫描均可取消、有数量边界、跳过云占位文件，删除默认进入系统废纸篓并可恢复。
...
### 存储分析与设备备份
Space Lens 会为用户选择的文件夹或已挂载磁盘建立可取消的容量目录树，支持逐层进入 Treemap；扫描不跟随符号链接，并跳过云占位文件。重复文件先按大小分桶，再进行抽样哈希和完整 SHA-256 确认；相似图片结合感知哈希、Vision 特征及清晰度、分辨率、曝光、人脸质量等可解释指标，并始终保留每组中建议的最佳副本。iPhone/iPad 备份功能读取受限的 MobileSync 本地元数据，按设备归类，可将单个已复核备份移入废纸篓并撤销。下载来源归类只展示本地 quarantine 元数据中的来源应用和域名，不显示完整 URL。
