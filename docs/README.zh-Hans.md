...
<p align="center">
  <b>夺回被焊死的 Mac 空间。一款零隐私泄露的 Mac 清理与卸载神器，专治存储焦虑。</b><br>
  <i>Reclaim your soldered SSD. A telemetry-free macOS cleaner & uninstaller that saves your storage—and your wallet.</i>
</p>
...
<p align="center">
  ...
  <a href="#-为什么我们需要这个项目--the-why">为什么我们需要它</a> -
  <a href="#-痛点对比--before-vs-after">痛点对比</a> -
  <a href="#-核心特性--key-features">核心特性</a> -
  <a href="#-典型场景--who-needs-this">典型场景</a> -
  <a href="#-同类对比--how-it-compares">同类对比</a> -
  <a href="#-我们的承诺--our-promise">我们的承诺</a> -
  <a href="#-功能深度详解--detailed-features">功能深度详解</a> -
  <a href="#-权限说明--permissions">权限说明</a>
</p>
...
*Get started and reclaim your space in 60 seconds:*
*在 60 秒内上手释放空间：*
...
*Or download the latest `.dmg` from [Releases](https://github.com/GravityPoet/AppSift/releases/latest) and drag it to `/Applications`.*

如果你是希望本地审查并自主编译的开发者，请参考 [进阶源码编译指南](#-进阶源码编译指南--advanced-source-build-guide)。
*For developers who want to inspect and compile locally, check out the [Advanced Source Build Guide](#-进阶源码编译指南--advanced-source-build-guide).*
...
因为苹果高昂的固态硬盘升级费让很多人只能选择 **256GB 的低配 Mac** (Mac mini, MacBook Air / Pro)，而市面上的商业清理软件大多采用**昂贵的订阅制** (predatory annual subscriptions)，默认收集并上传你的隐私数据，还喜欢用*“发现 47 GB 危险垃圾”*这种虚假警报 (FUD marketing) 来恐吓你续费。

AppSift 就是为此而生的终结者。它 **100% 免费**、基于 **MIT 协议开源**、完全**本地运行且绝无联网收集隐私** (offline-first & telemetry-free)。它不搞虚头巴脑的噱头，只用最干净、最诚实的方式，帮你把被开发缓存和残留垃圾蚕食的硬盘空间一寸一寸夺回来。

---

## ⚖️ 痛点对比 / Before vs. After

*The contrast makes it clear:*

| 没有 AppSift (痛苦现状 / The Pain) ❌ | 拥有 AppSift (爽快体验 / The Joy) 🎉 |
|---|---|
| 每年花 $40+ (200+元) 订阅清理软件，只为了点一下“一键清理”。 | **100% 免费 (MIT)**。把预算留给真正重要的地方。 |
...
| 各种开发缓存 (Xcode, Docker, Ollama) 动辄吃掉 50GB+ 空间。 | **开发者专属清理 (Developer-First)**。一键安全清理大型编译器、容器与本地模型缓存。 |
| 各种红色惊叹号和虚假警告弹窗恐吓你“系统处于危险中”以催促续费。 | **诚实扫描 (Honest Statistics)**。只展示真实路径，绝不使用恐吓营销。 |

---

## 🚀 核心特性 / Key Features

*Only the essential benefits:*

*   **⚡ 智能卸载与残留清理 / Smart Uninstaller & Deep Orphan Finder**
...
*   **⚙️ 开发者特供清理 / Developer-First Cache Purger**
    ...
    *Modern development stacks are storage vampires. AppSift scans and safely flushes giant cache folders from Xcode, Node, Docker, and local AI tools like Ollama and LM Studio.*

*   **🛡️ 安全安全，绝对掌控 / Bounded, Offline-First & Finder-Safe**
    没有任何后台上传或联网分析。所有删除操作默认移动到系统垃圾桶，随时可以后悔。核心系统路径被硬编码锁定保护，绝不会误伤系统。
    *Zero analytics, zero network tracking. Destructive actions use macOS Finder-semantic recycle to move files to Trash instead of permanent deletion, preventing accidental data loss.*
...
*   💻 **低配 Mac 钉子户 (Base Macbook / Mac Mini Owners):** 坚守 256GB 固态硬盘，每天都在为几百兆空间腾挪。
*   🛠️ **开发者与 AI 工程师 (Developers & AI Engineers):** 备受 Xcode 缓存、npm 依赖、Docker 镜像和本地大模型缓存折磨的人群。
*   🔒 **隐私洁癖者 (Privacy Enthusiasts):** 拒绝给闭源、带隐私收集的商业清理软件授予“完全磁盘访问权限 (Full Disk Access)”的人群。
...
|---|:---:|:---:|:---:|:---:|:---:|
| **价格 (Price)** | **免费 (Free)** | $40+/yr / 订阅制 | Free / 免费 | CLI free / GUI paid | Free / 免费 |
| **开源 (Open Source)** | **Yes (MIT)** | No / 闭源 | Source-available¹ | CLI only | No / 闭源 |
| **零遥测 (No Telemetry)** | **Yes / 是** | No / 否 | Yes / 是 | Yes / 是 | Yes / 是 |
| **无订阅 (No Subscription)** | **Yes / 是** | No / 否 | Yes / 是 | — | Yes / 是 |
| **原生界面 (Native Mac GUI)** | **Yes / 是** | Yes / 是 | Yes / 是 | Terminal-first | Yes / 是 |
| **源码可信度 (Provenance)** | **Source + checksums** | Vendor release | Project release | Source | Project release |
| **软件卸载 (App Uninstaller)** | **Yes / 是** | Yes / 是 | Yes / 是 | Partial | No / 否 |
| **回收站友好 (Trash-first)** | **Yes / 是** | Partial | Yes / 是 | Partial | No / 否 |
| **诚实不夸大 (Honest Stats)** | **Yes / 是** | No / 否 | n/a | n/a | n/a |
...
*A contract of trust built with the community:*

清理软件需要 macOS 最深层的权限——**完全磁盘访问权限 (Full Disk Access)**——然后去删除你的文件。这需要极高的信任度。以下是 AppSift 坚守的契约，你可以在源码中验证每一行：

*   **默认可恢复，不可恢复时明确提示 (Recoverable by Default)**：卸载、重置和清理操作全部采用 Finder 语义的回收机制 (`NSWorkspace.recycle`)，保存映射记录，可随时在回收站还原。还原绝不覆盖已有文件，管理员权限目标会提示通过 Finder 恢复以避免调用高权 Shell。
*   **绝对零遥测 (No Telemetry)**：无分析、无崩溃报告、无“匿名数据收集”。软件完全在本地离线运行。
*   **网络使用完全透明 (Explicit Network Use)**：只有在你手动点击“检查更新”时，才会发起受限的网络请求。检查包括 App Store 收据、Homebrew Cask信息以及 Github 官方 Releases 链接，绝不请求未知服务器或后台静默下载安装包。
*   **绝无虚假恐吓 (No Fake Urgency)**：不搞红色的警报弹窗，不弹虚假的“系统处于危险中”等夸大词汇，只呈现客观数据，由你自行决定。
*   **从不夸大其词 (No Overpromising)**：不吹嘘能“释放可清除空间”、“清理内存”或“加速 Mac”（这些在现代 macOS 上无法被第三方应用可靠实现）。
*   **清理前可完全预览 (User Review Before Removal)**：没有任何自动删除行为。每一项文件都支持在 Finder 中显示，并内置了针对敏感系统路径的硬编码拦截。
*   **完全可审计 (Auditable)**：所有扫描和判断逻辑在 [`AppSift/Services`](../AppSift/Services) 和 [`AppSift/Logic/Scanning`](../AppSift/Logic/Scanning) 中全部开源，接受全球开发者监督。

---

## 🛠️ 进阶源码编译指南 / Advanced Source Build Guide

*Compile locally to verify the build:*
...
## ⚙️ 功能深度详解 / Detailed Feature Breakdown
...
### Privacy Permissions / TCC 隐私权限审计
读取本机的 TCC (Transparency, Consent, and Control) 隐私配置记录并与 App `Info.plist` 中声明的权限缘由比对。AppSift 使用只读 SQLite 标志安全读取，绝不直接写入 TCC 数据库。用户点击“重置授权”仅触发系统自带的 `/usr/bin/tccutil reset` 指令。

### System Cleaner / 系统垃圾清理
...
> **关于 APFS "可清除空间 (Purgeable Space)"**：AppSift 会将可清除空间以直观图表展示以供知情，但绝不声称能“一键强制抹除”。因为 APFS 机制规定这部分空间应由 macOS 系统自发回收。我们选择提供诚实的数据，而不是虚假的噱头。

---

## 🛡️ 安全设计 / Security Features
