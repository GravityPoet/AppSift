...
<p align="center">
  <a href="https://github.com/GravityPoet/AppSift/releases/latest"><img src="https://img.shields.io/github/v/release/GravityPoet/AppSift?style=flat-square&label=%E3%83%80%E3%82%A6%E3%83%B3%E3%83%AD%E3%83%BC%E3%83%89" alt="最新リリース"></a>
...
[Releases](https://github.com/GravityPoet/AppSift/releases/latest) から最新の `.dmg` をダウンロードし、開いて AppSift を `/Applications` にドラッグします。

> 署名・公証の状態と SHA-256 は各リリースのノートを確認してください。

> AppSift は GravityPoet が独立して開発・保守する製品です。[PureMac](https://github.com/momenbasel/PureMac) のフォークから始まりました。原著作権とライセンスは [Attribution](../README.md#attribution) を参照してください。
...
- 正確なアプリ名、構造化バンドル ID、コンテナメタデータ、検証済みエンタイトルメントを証拠に使う範囲限定スキャン
- **3 段階の感度**: Strict（安全）、Enhanced（バランス重視）、Deep（徹底）
- 関連するすべてのファイルを表示: キャッシュ、設定、コンテナ、ログ、サポートファイル、ランチエージェント
- 各候補に一致した所有根拠を表示し、共有または不確かな項目は保護対象として分けて表示
- アンインストールはゴミ箱への移動だけを行い、移動済み・欠落・失敗・保護・復元済みを含むローカルレポートを保存。SHA-256 整合性チェック付きのパス JSON を書き出せ、ゴミ箱を変更せずに履歴を消去可能
- システムアプリ保護 — Apple のシステムアプリは自動的に対象外になります
...
主要なコンポーネント:
- **AppPathFinder** — キャンセル可能で範囲限定されたアプリファイルの証拠マッチャー
