...
(
  cd build
  shasum -a 256 "AppSift-$VERSION-self-signed.dmg" > "AppSift-$VERSION-self-signed.dmg.sha256"
  shasum -a 256 "AppSift-$VERSION-self-signed.zip" > "AppSift-$VERSION-self-signed.zip.sha256"
)
...
| 2026-07-29 | 1.0.5 / v1.0.5 | public `shasum -a 256 -c` against the downloaded checksum manifests | Both manifests referenced `build/AppSift-...`, so direct verification in the download directory reported `FAILED open or read`; DMG and ZIP container checks still passed | The package command generated manifests from the repository root and embedded the local staging-directory prefix | Immediately convert the Release to draft, regenerate basename-only manifests from inside `build/`, replace only those two manifests, redownload all assets, and republish only after direct verification passes | Generate portable checksum manifests from the artifact directory and always execute a fresh-download `shasum -c` before the Release remains public |
| 2026-07-29 | 1.0.5 / v1.0.5 | unauthenticated `curl -fsSIL` reachability check | One checksum-asset request reported `LibreSSL SSL_connect: SSL_ERROR_SYSCALL` while the other public endpoints and authenticated full downloads succeeded | A transient GitHub TLS handshake failure interrupted the single-attempt probe; a bounded retry immediately reached every endpoint and returned exact manifest contents | Re-run the same read-only public check with `set -e`, `--retry 3`, and `--retry-all-errors`; require HTTP 200 and exact checksum text | Public reachability probes must fail closed as a group and use bounded network retries instead of accepting the exit status of a later loop command |
| 2026-07-29 | 1.0.5 / v1.0.5 | full-tap `brew style GravityPoet/tap` and test-bot runs `30415671222` / `30415768760` | The AppSift cask passed its focused checks, but the first Tap-wide run failed because `Casks/openfind.rb` had a 106-character description | Focused AppSift validation did not expose a pre-existing style violation in another cask that the portable gate audits as part of the whole Tap | Shorten only the OpenFind customer description to 65 characters, keep its version/URL/checksum unchanged, rerun full-tap style/readall, and require both Homebrew runner jobs to pass in `30415768760` | Run full-tap style and readall before pushing a release cask; a target-cask pass is necessary but not sufficient for the Tap gate |
| 2026-07-29 | 1.0.5 / v1.0.5 | local full-tap Homebrew style validation | Homebrew installed `actionlint` and `shellcheck` as validator dependencies during the check | Full-tap style includes GitHub Actions validation and bootstraps missing validators on the host | Confirm both formulae were absent before the command from Homebrew's own `Would install` output, use them for validation, then uninstall exactly those two formulae and verify they are absent | Snapshot validator presence before any full-tap style/test-bot run and remove only dependencies proved to have been introduced by that run |
