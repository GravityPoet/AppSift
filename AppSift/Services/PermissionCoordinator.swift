...
/// - Auto-opens the Settings pane and reveals AppSift.app in Finder so users can
...
final class PermissionCoordinator: ObservableObject {
    ...
    enum PromptContext {
        ...
        case appReset(appName: String, failedCount: Int)
        ...
        var headline: String {
            ...
            case .appReset(let app, let n):
                return String(
                    format: String(localized: "Resetting %@: %lld file(s) need Full Disk Access"),
                    app,
                    Int64(n)
                )
            }
    ...
    /// Open System Settings AND reveal AppSift.app in Finder so the user can
    ...
    /// Reset AppSift's TCC entry and re-prime registration. Use when the user
    /// reports AppSift doesn't appear in the FDA list (typical after Homebrew
