...
/// Detects whether Full Disk Access (FDA) has been granted to AppSift.
...
struct FullDiskAccessManager: Sendable {
    ...
    /// the OS records AppSift as the requester and adds it to the FDA pane.
    ...
    /// Force AppSift to appear in the Full Disk Access list.
    ///
    ...
    /// *Finder* — not AppSift. So at launch we touch a small set of paths
    ...
    /// Reveal the running AppSift.app bundle in Finder so the user can drag
    ...
    /// Reset AppSift's TCC entries so the OS can re-register the bundle.
