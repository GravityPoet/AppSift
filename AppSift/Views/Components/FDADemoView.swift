...
struct FDADemoView: View {
    ...
    private var rows: [DemoRow] {
        [
            ...
            DemoRow(name: "AppSift", systemImage: "sparkles",
    ...
    private func rowView(_ row: DemoRow) -> some View {
        let highlight = row.isAppSift && (frame == .highlight)
        let succeeded = row.isAppSift && (frame == .granted || frame == .hold)

        return HStack(spacing: 12) {
            ZStack {
                RoundedRectangle(cornerRadius: 6, style: .continuous)
                    .fill(row.isAppSift ? Tint.blue.opacity(0.16) : Color.primary.opacity(0.08))
                Image(systemName: row.systemImage)
                    .font(.system(size: 13, weight: .semibold))
                    .foregroundStyle(row.isAppSift ? Tint.blue : .secondary)
            }
            .frame(width: 26, height: 26)

            Text(row.name)
                .font(.system(size: 13, weight: row.isAppSift ? .semibold : .regular))
