...
| Before AppSift (The Pain / 痛苦现状) ❌ | After AppSift (The Joy / 爽快体验) 🎉 |
|---|---|
| Paying $40+/year for disk utilities just to click "Clean". <br>*每年花 200+ 元订阅清理软件，只为了点一下“一键清理”。* | **100% Free (MIT)**. Keep your hard-earned budget. <br>***100% 免费 (MIT)**，把预算留给真正重要的地方。* |
| Creepy trackers & telemetry profiling your app usage history. <br>*清理软件在后台默默监控你的应用使用习惯和数据，隐私隐患重重。* | **Zero Telemetry**. Bounded, offline-first. It doesn't even know you exist. <br>***零遥测，零跟踪**。完全离线运行，应用甚至不需要知道你是谁。* |
| Leftovers from deleted apps (plists, containers, launchdaemons) silently rotting in `~/Library`. <br>*拖拽到垃圾桶的软件在 `~/Library` 下残留大量冷冻的废弃文件。* | **Deep Orphan Scan**. Matches bundles to trace and sweep every hidden byte. <br>***深度残留扫描 (Orphan Finder)**，深度匹配 bundle 连根拔起残留。* |
| Dev caches (Xcode, Node, Docker, Ollama) eating 50GB without you knowing. <br>*各种开发缓存 (Xcode, Docker, Ollama) 动辄吃掉几十 G 空间。* | **Developer-First Purger**. Safely wipes giant compiler, package, and LLM caches in one click. <br>***开发者专属清理**。一键安全清理大型编译器、容器与本地模型缓存。* |
| Dramatized red counters crying "Your Mac is in danger!" <br>*各种红色惊叹号和虚假警告弹窗恐吓你“系统处于危险中”以催促续费。* | **Honest Statistics**. Real paths, transparent rules, absolute control. <br>***诚实扫描**。只展示真实路径，绝不使用恐吓营销。* |
...
- **⚡ Smart Uninstaller & Deep Orphan Finder / 智能卸载与残留清理**
  ...
  * **ZH**: 普通的“拖入垃圾桶”会残留 70% 的废弃数据。AppSift 深入系统底层，匹配 bundle ID 和 containers，自动识别并清理所有关联 plist、启动项和日志，而且删除默认进回收站，支持一键安全还原。
...
## 🤝 Acknowledgments & Attribution / 致谢与声明
